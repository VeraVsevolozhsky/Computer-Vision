function [H,outparams] = DIMRnmf(X,y,inparams)

if nargin<3,
  inparams.npcs = 10;
end

[W,H]=inlinenmf(X,inparams.npcs,1);

outparams.W = W;
outparams.H = H;
outparams.npcs = inparams.npcs;

function [w,h]=inlinenmf(v,r,verbose)
%
...
if min(sum(v,2)) == 0
  warning('not all entries in a row can be zero, FIXING');
  v = v+0.01*min(min(v));
end
