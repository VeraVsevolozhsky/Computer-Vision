  ...
  int LIORCOUNTER = 0;
	...
	while(1)
	{
		...
		if(--counter == 0)
		{
			...
			//LIOR REMOVED THE DOT//info("."); info_flush();
			LIORCOUNTER++;
			if (LIORCOUNTER>15000)
			{
			  info("LIORBREAK"); info_flush();
			  break;
			}
		}
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	if (Cp==Cn)
		info("nu%3.3f, ", sum_alpha/(Cp*prob->l));
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	info("nu%3.3f, ",sum_alpha/(param->C*l));
...
decision_function svm_train_one(
	...
	double Cp, double Cn)
{
  static int numtimesbeingcalled = 0;
  numtimesbeingcalled++;
	...
	if (numtimesbeingcalled<10)
	  {
	    info("obj%3.3f,rho%3.3f,",si.obj,si.rho);
	  }
	// output SVs
...
void svm_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target)
{
	...
	for(i=0;i<nr_fold;i++)
	{
		   ...
		   (param->svm_type == C_SVC || param->svm_type == NU_SVC))
		{
			...
			for(j=begin;j<end;j++)
			  target[perm[j]] = svm_predict(submodel,prob->x[perm[j]],tmp);
		svm_destroy_model(submodel);
		free(subprob.x);
		free(subprob.y);
	}		
...
double svm_predict(const svm_model *model, const svm_node *x, double &maxdecvalue)
{
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR)
	...
	const svm_model *model, const svm_node *x, double *prob_estimates)
{
  double tmp;
	    ...
	    model->probA!=NULL && model->probB!=NULL)
	{
	  ...
	  return svm_predict(model, x, tmp);
}
