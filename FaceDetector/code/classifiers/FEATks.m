function [m_train, d] = FEATks(m_train, label, inparam)   %Kolmogorov-Smirnov feature selection
...
for i=1:size(XposTrain,1)
    [p, d.select(i)]=kstest2(XposTrain(i,:), XnegTrain(i,:));
                                                                                                                          %%%changes recently, used to be kstest below
end;
...
function [p, d] = kstest(v1, v2)
%KSTEST Kolmogorov-Smirnov test of two arrays.
%       KSTEST(V1, V2) gives the significance level of V1 and V2 being
%       sampled from the same distribution.
%       [PROB, D] = KSTEST(V1, V2) returns :
